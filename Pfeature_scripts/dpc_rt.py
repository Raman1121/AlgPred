...
def main(argv):
    ...
    order = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: dpc_rt.py -i inputfile -o outputfile -m number1 -n number2 -q order\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\ndpc_rt.py -i inputfile -o outputfile -m number1 -n number2 -q order\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 	
    ...
    dpc_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),int(sys.argv[10]))
