...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: acr_nt.py -i inputfile -o outputfile -n number -d lag\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nacr_nt.py -i inputfile -o outputfile -n number -d lag\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of N-Terminal residues\n')
            print('d : lag(<=30)\n') 			
    ...
    acr_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
