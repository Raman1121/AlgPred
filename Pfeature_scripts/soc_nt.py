...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: soc_nt.py -i inputfile -o outputfile -g gap -n number\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:g:n:",["ifile=","ofile=","g=","n="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nsoc_nt.py -i inputfile -o outputfile -g gap -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            print('gap : value of gap\n')	
            print('n : number of residues from N-terminal\n')			
    ...
    soc_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
