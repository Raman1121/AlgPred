...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: atc_ct.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\natc_ct.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of C-Terminal residues\n') 			
            sys.exit()
    ...
    atc_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]))
