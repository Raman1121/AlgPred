...
########################################atom#################
def atc_st(file,output,n) :
    filename, file_extension = os.path.splitext(file)
    file_atc=atc(file,n)
    file_bonds=bond(file,n)
    df3=file_atc.iloc[:,0:6]
    bb = []
    ...
    zz = pd.DataFrame(bb)    
    header = []
    head = ['C_perc_s','H_perc_s','N_perc_s','O_perc_s','S_perc_s']
...
def main(argv):
    ...
    number = 1
    if len(argv[1:]) == 0:
        print ("\nUsage: atc_st.py -i inputfile -o outputfile  -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\natc_st.py -i inputfile -o outputfile -m number1 -n number\n')
    ...
    atc_st(sys.argv[2],sys.argv[4],int(sys.argv[6]))
