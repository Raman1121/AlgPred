...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: apaac_nt.py -i inputfile -o outputfile -l lamda -n number -w weight\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\napaac_nt.py -i inputfile -o outputfile -l lamda -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('lamda : lamda value (gap)\n')	
            print('number : number of N-Terminal residues\n')		
    ...
    apaac_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
