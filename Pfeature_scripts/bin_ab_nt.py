...
def bin_ab_nt(file,outt,n):
    atom_bin_nt(sys.argv[2],sys.argv[4],n)
    bond_bin_nt(sys.argv[2],sys.argv[4],n)	
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_ab_nt.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_ab_nt.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of N-Terminal residues\n')
    ...
    bin_ab_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]))
