...
def main(argv):
    ...
    number2 = 1
    if len(argv[1:]) == 0:
        print ("\nUsage: qos_st.py -i inputfile -o outputfile  -g gap -n Number_of_splits -w weight\n")
    ...
    if len(argv[1:])<9:
        ...
        sys.exit()
    try:
      opts, args = getopt.getopt(argv,"i:o:g:n:w:",["ifile=","ofile=","n1=","n2=","n3="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            ...
            print('gap : is the value of the gap(1 for no gap)\n')
            ...
            n3 = float(sys.argv[10])
    qos_st(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
