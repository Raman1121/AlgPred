...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: qos_wp.py -i inputfile -o outputfile -g gap -w weight\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:g:w:",["ifile=","ofile=","g=","w="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nqos_wp.py -i inputfile -o outputfile -g gap -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            print('gap : value of gap\n')		
            print('weight : weight of a sequence\n')			
    ...
    qos_wp(sys.argv[2],sys.argv[4],int(sys.argv[6]),float(sys.argv[8]))
