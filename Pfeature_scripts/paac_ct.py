...
def aac_comp(file,v,w):
    ...
    for j in zz:
        ...
        c = c+1
    f.truncate()
def paac_ct(file,outt,lambdaval,v,w):
    filename, file_extension = os.path.splitext(file)
    paac_1(file,lambdaval,v,w=0.05)
    aac_comp(file,v,w)
    data1 = pd.read_csv(filename+".aac_paac_ct")
    ...
    os.remove(filename+".lam_ct")
    os.remove(filename+".aac_paac_ct")	
def main(argv):
    global inputfile
    global outputfile	
    inputfile = ''
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: paac_ct.py -i inputfile -o outputfile -l lamda -n number -w weight\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\npaac_ct.py -i inputfile -o outputfile -l lamda -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('lamda : lamda value (gap)\n')	
            print('number : number of C-Terminal residues\n')		
    ...
    paac_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
