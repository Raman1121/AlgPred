...
def bin_ab_wp(file,outt):
    atom_bin(sys.argv[2],sys.argv[4])
    bond_bin(sys.argv[2],sys.argv[4])	
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_ab_wp.py -i inputfile -o outputfile\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_ab_wp.py -i inputfile -o outputfile\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            sys.exit()
        elif opt in ("-i", "--ifile"):
    ...
    bin_ab_wp(sys.argv[2],sys.argv[4])
