...
#################################Transition#############
    ...
    df11 = pd.read_csv(outt+".ctd_ct_comp")
    df_1 = df11.iloc[:,:-1]
    zz = pd.DataFrame()
    ...
    zz.columns = head
    zz.to_csv(outt+".ctd_comp_ct", index=None, encoding='utf-8')
    head2 = []
    ...
    df12 = pd.read_csv(outt+".ctd_ct_trans")
    df_2 = df12.iloc[:,:-1]
    ss = pd.DataFrame()
    ...
    ss.columns = head2
    ss.to_csv(outt+".ctd_trans_ct", index=None, encoding='utf-8')
    head3 = []
    ...
    df_3 = pd.read_csv(outt+".ctd_ct_dist", sep=" ")
    rr = pd.DataFrame()
    ...
    rr.columns = head3
    rr.to_csv(outt+".ctd_dist_ct", index=None, encoding='utf-8')
    os.remove(outt+".ctd_ct_comp")
    os.remove(outt+".ctd_ct_trans")
    os.remove(outt+".ctd_ct_dist")
def main(argv):
    global inputfile
    global outputfile
    inputfile = ''
    ...
    number = 1
    if len(argv[1:]) == 0:
        print ("\nUsage: ctd_ct.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nctd_ct.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n')
            print('outputfile : is the file of feature vectors\n')
            print('number : number of N-Terminal residues\n')
    ...
    ctd_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]))
