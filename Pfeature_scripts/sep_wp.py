...
def sep_wp(filename,output):
    ...
    #file = open(os.path.splitext(filename)[0]+'.PCShannon_all','w')
    file = open(output,'w')	
    with file:
        writer = csv.writer(file);
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: sep_wp.py -i inputfile -o outputfile\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nsep_wp.py -i inputfile -o outputfile\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            sys.exit()
        elif opt in ("-i", "--ifile"):
