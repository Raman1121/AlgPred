...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: pcp_nt.py -i inputfile -o outputfile -m number\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:m:",["ifile=","ofile=","m="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\npcp_wp.py -i inputfile -o outputfile -m number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')		
            print('number : number of N-Terminal residues\n')				
            sys.exit()
            ...
            outputfile = arg
        elif opt in ("-m", "--number"):
            m = int(sys.argv[6])			
			
    
    pcp_nt(sys.argv[2],sys.argv[4],'NT',int(sys.argv[6]),int(sys.argv[6]))
