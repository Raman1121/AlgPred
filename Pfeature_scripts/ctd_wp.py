...
#################################Transition#############
    ...
    zz.columns = head
    zz.to_csv(outt+".ctd_comp", index=None, encoding='utf-8')
    head2 = []
    ...
    ss.columns = head2
    ss.to_csv(outt+".ctd_trans", index=None, encoding='utf-8')
    head3 = []
    ...
    df_3 = pd.read_csv(outt+".dist", sep=" ")
    rr = pd.DataFrame()
    ...
    rr.columns = head3
    rr.to_csv(outt+".ctd_dist", index=None, encoding='utf-8')
    os.remove(outt+".comp")
    os.remove(outt+".trans")
    os.remove(outt+".dist")
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: ctd_wp.py -i inputfile -o outputfile\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nctd_wp.py -i inputfile -o outputfile\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            sys.exit()
        elif opt in ("-i", "--ifile"):
