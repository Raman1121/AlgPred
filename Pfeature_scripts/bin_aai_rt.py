...
AAIndex = pd.read_csv('Data/z_aaindex.csv',index_col='INDEX');
...
def bin_aai_rt(filename,outt,num1,num2):
...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_aai_rt.py -i inputfile -o outputfile -m number1 -n number2\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_aai_rt.py -i inputfile -o outputfile -m number1 -n number2\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 			
    ...
    bin_aai_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
