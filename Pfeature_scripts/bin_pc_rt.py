...
def binary_profile_all(file,m,n,featureNumb):

    seq=file;
    seq = split(seq,'rest',m,n);
    l = len(seq);
    #print('length is',l)
...
def bin_pc_rt(filename,outt,m,n):
    ...
    for i in range(len(seq)):
        ...
        for j in range(25):
            ...
            binprof = binary_profile_all([seq[i]],m,n,j);
            out1.extend(binprof)
            k=k+1;
        out.append(out1);
        out1=[];
...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_pc_rt.py-i inputfile -o outputfile -m number1 -n number2\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_pc_rt.py-i inputfile -o outputfile -m number1 -n number2\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 			
    ...
    bin_pc_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
