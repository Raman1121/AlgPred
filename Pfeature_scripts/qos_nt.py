...
def qos_nt(file,outt,gap,n,w=0.1):
    file1 = nt(file,n)
    ff = []
    filename, file_extension = os.path.splitext(file)
    df2=file1
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: qos_nt.py -i inputfile -o outputfile -g gap -n number -w weight\n")
        ...
        print('n : number of residues from N-terminal\n')		
        print('weight : weight of a sequence\n')		
        sys.exit()	
		
    try:
      opts, args = getopt.getopt(argv,"i:o:g:n:w:",["ifile=","ofile=","g=","n=","w="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nqos_nt.py -i inputfile -o outputfile -g gap -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            print('gap : value of gap\n')	
            print('n : number of residues from N-terminal\n')			
    ...
    qos_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
