...
#################################Transition#############
    ...
    df11 = pd.read_csv(outt+".ctd_rest_comp")
    df_1 = df11.iloc[:,:-1]
    zz = pd.DataFrame()
    ...
    zz.columns = head
    zz.to_csv(outt+".ctd_comp_rest", index=None, encoding='utf-8')
    head2 = []
    ...
    df12 = pd.read_csv(outt+".ctd_rest_trans")
    df_2 = df12.iloc[:,:-1]
    ss = pd.DataFrame()
    ...
    ss.columns = head2
    ss.to_csv(outt+".ctd_trans_rest", index=None, encoding='utf-8')
    head3 = []
    ...
    df_3 = pd.read_csv(outt+".ctd_rest_dist", sep=" ")
    rr = pd.DataFrame()
    ...
    rr.columns = head3
    rr.to_csv(outt+".ctd_dist_rest", index=None, encoding='utf-8')
    os.remove(outt+".ctd_rest_comp")
    os.remove(outt+".ctd_rest_trans")
    os.remove(outt+".ctd_rest_dist")
...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: ctd_rt.py -i inputfile -o outputfile -m number1 -n number2\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nctd_rt.py -i inputfile -o outputfile -m number1 -n number2\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 			
    ...
    ctd_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
