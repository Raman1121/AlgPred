...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: acr_st.py -i inputfile -o outputfile -n number -d lag\n")
        ...
        print('d : is the lag(<=30) and it always should be less than the length of the sequence (whole or splitted)\n') 			
        sys.exit()	
		
    try:
      opts, args = getopt.getopt(argv,"i:o:n:d:",["ifile=","ofile=","n=","d="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nacr_st.py -i inputfile -o outputfile -n number -d lag\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of splits\n')
            print('d : is the lag(<=30) and it always should be less than the length of the sequence (whole or splitted)\n') 			
    ...
    acr_st(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
