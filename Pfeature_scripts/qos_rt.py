...
def qos_rt(file,outt,gap,n,c,w):
    file1 = rest(file,n,c)
    ff = []
    filename, file_extension = os.path.splitext(file)
    df2=file1
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: qos_rt.py -i inputfile -o outputfile -g gap -n number -w weight\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:g:m:n:w:",["ifile=","ofile=","g=","m=","n=","w="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nqos_rt.py -i inputfile -o outputfile -g gap -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            print('gap : value of gap\n')
            print('m : number of residues from N-terminal\n')			
    ...
    qos_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),int(sys.argv[10]),float(sys.argv[12]))
