...
This function gives Shannon's entropy of all physico-chemical properties of the C terminal sequence ('CT')  for all the comma separated sequences input by the user
...
arg2 = Number of residues to be taken
...
headers = ['Positively charged_ct',
'Negatively charged_ct',
'Neutral charged_ct',
'Polarity_ct',
'Non polarity_ct',
'Aliphaticity_ct',
...
'Hydroxylic_ct',
'Sulphur content_ct',
'Secondary Structure(Helix)_ct',
'Secondary Structure(Strands)_ct',
'Secondary Structure(Coil)_ct',
'Solvent Accessibilty (Buried)_ct',
'Solvent Accesibilty(Exposed)_ct',
'Solvent Accesibilty(Intermediate)_ct',
'Tiny_ct',
'Small_ct',
'Large_ct'];
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: sep_ct.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nsep_ct.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of C-Terminal residues\n') 			
            sys.exit()
    ...
    sep_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]))
