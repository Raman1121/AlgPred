import os
import sys
import pandas as pd 
import getopt
from itertools import repeat

def split(file,n):
    filename, file_extension = os.path.splitext(file)
    df1 = pd.read_csv(file, header = None)
    df2 = pd.DataFrame(df1[0].str.upper())
    k1 = []
    for w in range(0,len(df2)):
        s = 0
        k2 = []
        r = 0
        if len(df2[0][w])%n == 0:
            k2.extend(repeat(int(len(df2[0][w])/n),n))
        else:
            r = int(len(df2[0][w])%n)
            k2.extend(repeat(int(len(df2[0][w])/n),n-1))
            k2.append((int(len(df2[0][w])/n))+r)
        for j in k2:
            df3 = df2[0][w][s:j+s]
            k1.append(df3)
            s = j+s
    df4 = pd.DataFrame(k1)
    #df4.to_csv(filename+".split", index = None, header = False, encoding = 'utf-8')
    return df4

def bond(file,n) :
    df = split(file,n)
    tota = []
    hy = []
    Si = []
    Du = []
    b1 = []
    b2 = []
    b3 = []
    b4 = []
    bb = pd.DataFrame()
    filename, file_extension = os.path.splitext(file)
    #df12 = pd.read_csv(file, header = None)
    #df = pd.DataFrame(df12[0].str.upper())
    bonds=pd.read_csv("Data/bonds.csv")
    for i in range(0,len(df)) :
        tot = 0
        h = 0
        S = 0
        D = 0
        tota.append([i])
        hy.append([i])
        Si.append([i])
        Du.append([i])
        for j in range(0,len(df[0][i])) :
            temp = df[0][i][j]
            for k in range(0,len(bonds)) :
                if bonds.iloc[:,0][k] == temp :
                    tot = tot + bonds.iloc[:,1][k]
                    h = h + bonds.iloc[:,2][k]
                    S = S + bonds.iloc[:,3][k]
                    D = D + bonds.iloc[:,4][k]
        tota[i].append(tot)
        hy[i].append(h)
        Si[i].append(S)
        Du[i].append(D)
    for m in range(0,len(df)) :
        b1.append(tota[m][1])
        b2.append(hy[m][1])
        b3.append(Si[m][1])
        b4.append(Du[m][1])
    
    bb["total_bonds"] = b1 
    bb["hydrogen_bonds"] = b2
    bb["single_bond"] = b3
    bb["double_bond"] = b4 
    
    #bb.to_csv(filename+".bonds_info")
    return bb

###########################atom###############
def atc(file,n) :
    test1 = split(file,n)
    atom=pd.read_csv("Data/atom.csv",header=None)
    filename, file_extension = os.path.splitext(file)
    at=pd.DataFrame()
    i = 0
    C_atom = []
    H_atom = []
    N_atom = []    
    O_atom = []
    S_atom = []
 
    while i < len(atom):
        C_atom.append(atom.iloc[i,1].count("C"))
        H_atom.append(atom.iloc[i,1].count("H"))
        N_atom.append(atom.iloc[i,1].count("N"))
        O_atom.append(atom.iloc[i,1].count("O"))
        S_atom.append(atom.iloc[i,1].count("S"))
        i += 1
    atom["C_atom"]=C_atom  
    atom["O_atom"]=O_atom  
    atom["H_atom"]=H_atom  
    atom["N_atom"]=N_atom 
    atom["S_atom"]=S_atom 
##############read file ##########	
    #df12 = pd.read_csv(file, header = None)
    #test1 = pd.DataFrame(df12[0].str.upper())
    #test1 = pd.read_csv(file,header=None)
    dd = []
    for i in range(0, len(test1)):
        dd.append(test1[0][i].upper())
    test = pd.DataFrame(dd)
    cousplit_C = 0
    cousplit_H = 0
    cousplit_N = 0
    cousplit_O = 0
    cousplit_S = 0
    cousplit = 0
    i1 = 0
    j = 0
    k = 0
    C_split = []
    H_split = []
    N_split = []
    O_split = []
    S_split = []
    while i1 < len(test) :
        while j < len(test[0][i1]) :
            while k < len(atom) :
                if test.iloc[i1,0][j]==atom.iloc[k,0].replace(" ","") :
                    cousplit_C = cousplit_C + atom.iloc[k,2]
                    cousplit_H = cousplit_H + atom.iloc[k,3]
                    cousplit_N = cousplit_N + atom.iloc[k,4]
                    cousplit_O = cousplit_O + atom.iloc[k,5]
                    cousplit_S = cousplit_S + atom.iloc[k,6]
                #cousplit = cousplit_C + cousplit_H + cousplit_S + cousplit_N + cousplit_O
                k += 1
            k = 0
            j += 1
        C_split.append(cousplit_C)
        H_split.append(cousplit_H)
        N_split.append(cousplit_N)
        O_split.append(cousplit_O)
        S_split.append(cousplit_S)
        cousplit_C = 0
        cousplit_H = 0
        cousplit_N = 0
        cousplit_O = 0
        cousplit_S = 0
        j = 0
        i1 += 1
    test["C_cousplit"]=C_split  
    test["H_cousplit"]=H_split  
    test["N_cousplit"]=N_split 
    test["O_cousplit"]=O_split  
    test["S_cousplit"]=S_split     

    split_total = []
    m = 0
    while m < len(test) :
        split_total.append(test.iloc[m,1] + test.iloc[m,2] + test.iloc[m,3] + test.iloc[m,4] + test.iloc[m,5])
        m += 1
    test["cousplit"]=split_total    
##########final output#####
    final = pd.DataFrame()
    n = 0
    p = 0
    C_p = []
    H_p = []
    N_p = []
    O_p = []
    S_p = []
    while n < len(test):
        C_p.append((test.iloc[n,1]/test.iloc[n,6])*100)
        H_p.append((test.iloc[n,2]/test.iloc[n,6])*100)
        N_p.append((test.iloc[n,3]/test.iloc[n,6])*100)
        O_p.append((test.iloc[n,4]/test.iloc[n,6])*100)
        S_p.append((test.iloc[n,5]/test.iloc[n,6])*100)
        n += 1
    final["C_perc"] = C_p
    final["H_perc"] = H_p
    final["N_perc"] = N_p
    final["O_perc"] = O_p
    final["S_perc"] = S_p

    #(final.round(2)).to_csv(filename+".atc")
    return final.round(2)

########################################atom_bond#################
def abc_st(file,output,n) :
    filename, file_extension = os.path.splitext(file)
    file_atc=atc(file,n)
    file_bonds=bond(file,n)
    df1=file_atc.iloc[:,0:6]
    df2=file_bonds.iloc[:,0:5]
    df3 = pd.concat([df1,df2],axis=1)
    bb = []
    for i in range(0,len(df3),n):
        aa = []
        for j in range(n):
            aa.extend(df3.loc[i+j])
        bb.append(aa)
    zz = pd.DataFrame(bb)    
    header = []
    head = ['C_perc_s','H_perc_s','N_perc_s','O_perc_s','S_perc_s','total_bonds_s','hydrogen_bonds_s','single_bond_s','double_bond_s']
    for e in range(1,n+1):
        for t in head:
            header.append(t+str(e))
    zz.columns=header
    zz.to_csv(output, index=None)
def main(argv):
    global inputfile
    global outputfile
    inputfile = ''
    outputfile = ''
    number = 1
    if len(argv[1:]) == 0:
        print ("\nUsage: abc_st.py -i inputfile -o outputfile  -n number\n")
        print("-i: Input file in single line format\n-o: Output file\n-n: Number of splits")
        sys.exit()

    if len(argv[1:])<5:
        print ("\nUsage: abc_st.py -i inputfile -o outputfile  -n number\n")
        print("-i: Input file in single line format\n-o: Output file\n-n: Number of splits")
        print("\nInputError: You have provided lesser number of arguments. \n")
        sys.exit()
    try:
      opts, args = getopt.getopt(argv,"i:o:n:",["ifile=","ofile=","n="])
    except getopt.GetoptError:
        print ("\nUsage: abc_st.py -i inputfile -o outputfile -n number\n")
        sys.exit(2)
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nabc_st.py -i inputfile -o outputfile -m number1 -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n')
            print('outputfile : is the file of feature vectors\n')
            print('number1 : Number of splits\n')
            sys.exit()
        elif opt in ("-i", "--ifile"):
            inputfile = arg
        elif opt in ("-o", "--ofile"):
            outputfile = arg
        elif opt in ("-n", "-n"):
            n = int(sys.argv[6])

    abc_st(sys.argv[2],sys.argv[4],int(sys.argv[6]))

if __name__ == '__main__':
    #print(sys.argv)
    main(sys.argv[1:])
