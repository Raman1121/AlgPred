...
def geary(prop,seq,d):
    if (d<=30):
        ...
        print("Error!: d should be less than equal to 30")
        return
def acr_ct(filename,outt,n,d):
    if (d<=30):
        seq_data=splitstring_C(filename, n)
...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: acr_ct.py -i inputfile -o outputfile -n number -d lag\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nacr_ct.py -i inputfile -o outputfile -n number -d lag\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of N-Terminal residues\n')
            print('d : lag(<=30)\n') 			
    ...
    acr_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
