...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: acr_wp.py -i inputfile -o outputfile -d lag\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:d:",["ifile=","ofile=","dfile="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nacr_wp.py -i inputfile -o outputfile -d lag\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')		
            print('d : lag(<=30)\n')			
            sys.exit()
    ...
    acr_wp(sys.argv[2],sys.argv[4],int(sys.argv[6]))
