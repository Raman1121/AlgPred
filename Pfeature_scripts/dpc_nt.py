...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: dpc_nt.py -i inputfile -o outputfile -n number -q order\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\ndpc_nt.py -i inputfile -o outputfile -n number -q order\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')	
            print('order : is order of dipeptide\n')			
    ...
    dpc_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
