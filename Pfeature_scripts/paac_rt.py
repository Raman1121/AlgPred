...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: paac_rt.py -i inputfile -o outputfile -l lamda -m number1 -n number2 -w weight\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\npaac_rt.py -i inputfile -o outputfile -l lamda -m number1 -n number2 -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('lamda : lamda value (gap)\n')	
            print('number1 : number of N-Terminal residues\n')		
    ...
    paac_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),int(sys.argv[10]),float(sys.argv[12]))
