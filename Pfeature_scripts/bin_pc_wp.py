...
def binary_profile_all(file,n,featureNumb):
...
def bin_pc_wp(filename,outt):
    ...
    for i in range(len(seq)):
        ...
        for j in range(25):
            ...
            binprof = binary_profile_all([seq[i]],10,j);
            out1.extend(binprof)
            k=k+1;
        out.append(out1);
        out1=[];
...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_pc_wp.py -i inputfile -o outputfile\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_pc_wp.py -i inputfile -o outputfile\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            sys.exit()
        elif opt in ("-i", "--ifile"):
    ...
    bin_pc_wp(sys.argv[2],sys.argv[4])
