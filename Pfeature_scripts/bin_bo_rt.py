...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_bo_rt_rest.py -i inputfile -o outputfile -m number1 -n number2\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_bo_rt_rest.py -i inputfile -o outputfile -m number1 -n number2\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 			
    ...
    bin_bo_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
