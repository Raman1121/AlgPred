...
def main(argv):
    ...
    #option = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: qos_ct.py -i inputfile -o outputfile -g gap -n number -w weight\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nqos_ct.py -i inputfile -o outputfile -g gap -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')			
            print('gap : value of gap\n')	
            print('n : number of residues from C-terminal\n')			
    ...
    qos_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
