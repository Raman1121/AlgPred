...
AAIndex = pd.read_csv('Data/aaindex_X.csv',index_col='INDEX');
AAIndexNames = pd.read_csv('Data/aaind.txt',header=None);
...
def main(argv):
    ...
    window_size = 3
    if len(argv[1:]) == 0:
        print ("Usage:python pat_aai.py -i inputFile -a file_of_amino_acid_indices -w window_size -o outputFile\n")
        print ('-i\tInputFile\n-a\tFile having the list of amino acid indices name\n-w\tWindow size to create motifs\n-o\tOutput Motif file\n')
        sys.exit()
    ...
    if len(argv[1:])<7:
        ...
        sys.exit()
    try:
      opts, args = getopt.getopt(argv,"i:a:w:o:",["ifile=","a=","w=","ofile="])
    except getopt.GetoptError:
        print ("\nUsage:python pat_aai.py -i inputFile -a file_of_amino_acid_indices -w window_size -o outputFile\n")
        sys.exit(2)
    for opt, arg in opts:
    ...
    pat_aai(sys.argv[2],sys.argv[4],int(sys.argv[6]),sys.argv[8])
