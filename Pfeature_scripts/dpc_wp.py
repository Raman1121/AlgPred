...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: dpc_wp.py -i inputfile -o outputfile -q order\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\ndpc_wp.py -i inputfile -o outputfile -q order\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')	
            print('order : is order of dipeptide\n')			
            sys.exit()
    ...
    dpc_wp(sys.argv[2],sys.argv[4],int(sys.argv[6]))
