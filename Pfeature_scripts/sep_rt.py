...
This function gives Shannon's entropy of all physico-chemical properties of subsequence from mth to nth position(both inclusive)for all the comma separated sequences input by the user
...
arg2 = m; Start position

arg3 = n; End position

arg4 = output file containing Shannon's entropy of all physico-chemical properties of all the comma separated sequences
...
headers = ['Positively charged_rest',
'Negatively charged_rest',
'Neutral charged_rest',
'Polarity_rest',
'Non polarity_rest',
'Aliphaticity_rest',
'Cyclic_rest',
...
'Neutral_rest',
'Hydroxylic_rest',
'Sulphur content_rest',
'Secondary Structure(Helix)_rest',
'Secondary Structure(Strands)_rest',
'Secondary Structure(Coil)_rest',
'Solvent Accessibilty (Buried)_rest',
'Solvent Accesibilty(Exposed)_rest',
'Solvent Accesibilty(Intermediate)_rest',
'Tiny_rest',
'Small_rest',
'Large_rest',
'z1',
...
def sep_rt(filename,output,m,n):
...
#ans=sep_rt(sys.argv[1],int(sys.argv[2]),int(sys.argv[3]))
...
def main(argv):
    ...
    number2 = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: sep_rt.py -i inputfile -o outputfile -m number1 -n number2\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nsep_rt.py -i inputfile -o outputfile -m number1 -n number2\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
            print('number2 : number of C-Terminal residues\n') 			
    ...
    sep_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]))
