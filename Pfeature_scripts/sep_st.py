...
This function gives Shannon's entropy of all physico-chemical properties of the entire sequence ('all')  for all the comma separated sequences input by the user
...
arg2 = output file containing Shannon's entropy of all physico-chemical properties of all the comma separated sequences
...
headers = ['Positively charged_split',
'Negatively charged_split',
'Neutral charged_split',
'Polarity_split',
'Non polarity_split',
'Aliphaticity_split',
'Cyclic_split',
...
'Basicity_split',
'Neutral (ph)_split',
'Hydrophobicity_split',
'Hydrophilicity_split',
'Neutral_split',
'Hydroxylic_split',
'Sulphur content_split',
'Secondary Structure(Helix)_split',
'Secondary Structure(Strands)_split',
'Secondary Structure(Coil)_split',
'Solvent Accessibilty (Buried)_split',
'Solvent Accesibilty(Exposed)_split',
'Solvent Accesibilty(Intermediate)_split',
'Tiny_split',
'Small_split',
'Large_split'];
...
def sep_st(filename,output,parts):
    ...
    newseq = [splits1(seq[i],parts) for i in range(len(seq))]
    seq  = list(itertools.chain.from_iterable(newseq))
    #print(newseq)
    comp = phyChem(seq);
    ...
    headerNew=[]
    for i in range(parts):
            headerNew = headerNew+headers[0:25];
            #print(headerNew)
    entropy.append(headerNew)
    seqEntropy = [];
    #print(new);
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: sep_st.py -i inputfile -o outputfile -n number\n")
        sys.exit()	
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nsep_st.py -i inputfile -o outputfile -n number\n')
            sys.exit()
        elif opt in ("-i", "--ifile"):
            inputfile = arg
        elif opt in ("-o", "--ofile"):
    ...
    sep_st(sys.argv[2],sys.argv[4],int(sys.argv[6]))
