...
def apaac_1_rest(file,lambdaval,v,c,w):
    #w=0.05
    df1 = rest(file,v,c)
    filename, file_extension = os.path.splitext(file)
...
def aac_comp(file,v,c,w):
    ...
    for j in zz:
        ...
        c = c+1
    f.truncate()
def apaac_rt(file,outt,lambdaval,v,c,w):
    filename, file_extension = os.path.splitext(file)
    apaac_1_rest(file,lambdaval,v,c,w)
    aac_comp(file,v,c,w)
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: apaac_rt.py -i inputfile -o outputfile -l lamda -m number1 -n number2 -w weight\n")
    ...
    try:
      opts, args = getopt.getopt(argv,"i:o:l:m:n:w:",["ifile=","ofile=","l=","m=","n=","w="])
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\napaac_rt.py -i inputfile -o outputfile -l lamda -m number1 -n number2 -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('lamda : lamda value (gap)\n')	
            print('number1 : number of N-Terminal residues\n')		
    ...
    apaac_rt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),int(sys.argv[10]),float(sys.argv[12]))
