...
def paac_1(file,lambdaval,v,w=0.05):
    ...
    for i in range(0,len(df2)):
        ...
        df4 = pd.DataFrame(df3)
        df4.to_csv(filename+".nt", index = None, header = False, encoding = 'utf-8')
    df1 = pd.read_csv(filename+".nt", header = None)
    dd = []
    cc = []
    pseudo = []
    aa = {}
...
def aac_comp(file,v,w):
    ...
    for j in zz:
        ...
        c = c+1
    f.truncate()
def paac_nt(file,outt,lambdaval,v,w):
    filename, file_extension = os.path.splitext(file)
    paac_1(file,lambdaval,v,w)
    aac_comp(file,v,w)
    data1 = pd.read_csv(filename+".aac_paac_nt")
    data2 = pd.read_csv(filename+".lam_nt")
    data3 = pd.concat([data1.iloc[:,:-1],data2], axis = 1).reset_index(drop=True)
    data3.to_csv(outt, index = None)
    os.remove(filename+".more")
    os.remove(filename+".lam_nt")
    os.remove(filename+".aac_paac_nt")
    os.remove(filename+".nt")
def main(argv):
    global inputfile
    global outputfile	
    inputfile = ''
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: paac_nt.py -i inputfile -o outputfile -l lamda -n number -w weight\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\npaac_nt.py -i inputfile -o outputfile -l lamda -n number -w weight\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('lamda : lamda value (gap)\n')	
            print('number : number of N-Terminal residues\n')		
    ...
    paac_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]),int(sys.argv[8]),float(sys.argv[10]))
