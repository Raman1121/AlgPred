...
def ctc_nt(filename,outt,num):
    #num=10
    df = pd.DataFrame(columns=['Sequence','Triad:Frequency'])
    data=splitstring_N(filename,num)
    for i in range(len(data)):
        data1=''
        data1=str(data[i])
        data1=data1.upper()
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: ctc_nt.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nctc_nt.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number1 : number of N-Terminal residues\n')
    ...
    ctc_nt(sys.argv[2],sys.argv[4],int(sys.argv[6]))
