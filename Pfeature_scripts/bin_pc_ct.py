...
def binary_profile_ct(file,n,featureNumb):
...
def bin_pc_ct(filename,outt,n):
    ...
    for i in range(len(seq)):
        ...
        for j in range(25):
            ...
            binprof = binary_profile_ct([seq[i]],10,j);
            out1.extend(binprof)
            k=k+1;
        out.append(out1);
        out1=[];
...
def main(argv):
    ...
    number = 1	
    if len(argv[1:]) == 0:
        print ("\nUsage: bin_pc_ct.py -i inputfile -o outputfile -n number\n")
    ...
    for opt, arg in opts:
        if opt == '--help' or opt == '--h':
            print ('\nbin_pc_ct.py -i inputfile -o outputfile -n number\n')
            print('inputfile : file of peptide/protein sequences for which descriptors need to be generated\n') 
            print('outputfile : is the file of feature vectors\n')
            print('number : number of N-Terminal residues\n')
    ...
    bin_pc_ct(sys.argv[2],sys.argv[4],int(sys.argv[6]))
